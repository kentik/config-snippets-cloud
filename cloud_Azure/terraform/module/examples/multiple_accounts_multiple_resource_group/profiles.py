...
def load_incomplete_profiles(file_path: str) -> List[AzureProfile]:
    ...
    config = configparser.ConfigParser()
    try:
        # note: config.read silently ignores the fact that file_path doesn't exist,
        # resulting in empty config dict and no error reported
...
def raise_on_invalid_storage(profile: AzureProfile) -> None:
    ...
    if len(profile.storage_account_names) != len(profile.resource_group_names):
        msg = (
            ...
            f"'storage_account_names' is specified but has different item count than 'resource_group_names'"
        )
